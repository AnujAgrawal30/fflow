...
module.exports = (req, res, next) => {
    ...
    res.apiErrorResponse = (error) => {
        ...
        //Rapyd error
        if(error && error.status && error.status.status === 'ERROR') {

            switch (error.status.error_code) {
                case 'NOT_ENOUGH_FUNDS':
                    return getForm(403, null, i18n.__("INSUFFICIENT_FUNDS_IN_DEBITED_WALLET"));
                case 'USER_ALREADY_EXISTS':
                    return getForm(403, null, i18n.__("USER_IS_NOT_UNIQUE"));
                    case 'INVALID_PHONE_NUMBER':
                    return getForm(403, null, i18n.__("INVALID_PHONE_NUMBER"));
            }
