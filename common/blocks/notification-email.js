...
class Email extends AbstractBlock {
    ...
    async onAction() {
        ...
        if(!this.properties['name'] && typeof document === 'object')
            return Modal.Toast('error', 'Cannot send email: Missing recipient name');
        else if(!this.properties['email'] && typeof document === 'object')
            return Modal.Toast('error', 'Cannot send email: Missing recipient email');
        else if(!this.properties['message'] && typeof document === 'object')
            return Modal.Toast('error', 'Cannot send email: Missing message');
